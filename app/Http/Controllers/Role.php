...
class Role extends Controller
{
    ...
    private function get_menus(){
        ...
        foreach($main_menus as $main_menu){
            ...
            $menu_array[$main_menu->id]['childs'] = $all_sub_menu_ids;

            foreach($sub_menus as $sub_menu){
                
                $menu_array[$main_menu->id]['sub_menu'][$sub_menu->id] = [
                    "menu_key" => $sub_menu->id,
                    "label" => $sub_menu->label,
                    "type" => $sub_menu->type,
                    "sub_items" => [$sub_menu->id]
                ];
                $menu_array[$main_menu->id]['sub_menu'][$sub_menu->id]['siblings'] = $all_sub_menu_ids;
                ...
                if($sub_menu->type == 'SUB_MENU'){
                    ...
                    $menu_array[$main_menu->id]['childs'] = array_merge($menu_array[$main_menu->id]['childs'], $all_action_menu_ids);
                    $menu_array[$main_menu->id]['sub_menu'][$sub_menu->id]['childs'] = $all_action_menu_ids;

                    foreach ($action_menus as $action_menu) {
                        $menu_array[$main_menu->id]['sub_menu'][$sub_menu->id]['actions'][] = [

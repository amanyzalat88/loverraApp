    ...
    <div class="container pt-3 entry_form">
        <div class="d-flex justify-content-center pt-5">
            <div class="col-sm-12 col-md-6 col-lg-4 col-lg-4">
                ...
                <label class="text-display-0 mb-3">Sign in</label>
                <p v-html="server_errors" v-bind:class="[error_class]"></p>
                {{ message }}
                <form @submit.prevent="submit_form" class="mb-3">
                    <div class="form-group">
                        <label for="email">Email</label>
                        <input type="email" name="email" v-model="email" v-validate="'required|email'" class="form-control form-control-lg" placeholder="Please enter your registered email"  autocomplete="off">
                        <span v-bind:class="{ 'error' : errors.has('email') }">{{ errors.first('email') }}</span> 
                    </div>
                    <div class="form-group">
                        <label for="password">Password</label>
                        <input type="password" name="password" v-model="password" v-validate="'required'" class="form-control form-control-lg" placeholder="Please enter your password">
